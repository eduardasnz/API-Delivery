...
function EnsureAuthenticated(request2, response, next) {
  try {
    const authHeaders = request2.headers.authorization;
    if (!authHeaders) {
...
describe("user controller", () => {
  ...
  it("should be able to create a new user", async () => {
    const response = await (0, import_supertest.default)(app).post("/users").send({ name: "Teste Example", email: "Teste@example.com", password: "123456" });
    expect(response.status).toBe(201);
    expect(response.body).toHaveProperty("id");
    id_user = response.body.id;
  });
  it("should throw a error if email already exists", async () => {
    const response = await (0, import_supertest.default)(app).post("/users").send({ name: "Teste Two", email: "Teste@example.com", password: "123456" });
    expect(response.status).toBe(400);
    expect(response.body.message).toBe("User alreary exists.");
  });
  it("should a throw validation error if email is invalid", async () => {
    const response = await (0, import_supertest.default)(app).post("/users").send({ name: "Teste Example", email: "example-com", password: "123456" });
    expect(response.status).toBe(400);
